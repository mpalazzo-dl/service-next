...
const pageBodyPreviewFilePath = join(
  __dirname,
  "..",
  "src",
  "libs",
...
export interface Cf${reactCasedName}ClientProps extends CfFetchById {}

export const Cf${reactCasedName}Client = ({
  id,
  preview,
  lang,
}: Cf${reactCasedName}ClientProps) => {
...
const updatePreviewPageBodyComponent = (reactCasedName: string) => {
  if (!existsSync(pageBodyPreviewFilePath)) {
    console.error(
  ...
  try {
    let fileContent = readFileSync(pageBodyPreviewFilePath, "utf8");
    ...
    if (!fileContent.includes(importStatement)) {
      ...
      console.log(`✅ Added "${reactCasedName}" to preview imports.`);
    } else {
      console.log(
        `ℹ️ Component "${reactCasedName}" is already in the preview imports.`,
      );
    }
    ...
    if (!fileContent.includes(`case "${reactCasedName}":`)) {
      ...
      console.log(`✅ Added "${reactCasedName}" to preview switch statement.`);
    } else {
      console.log(
        `ℹ️ Component "${reactCasedName}" is already in the preview switch statement.`,
      );
    }
    ...
    writeFileSync(pageBodyPreviewFilePath, fileContent, "utf8");
    console.log(
